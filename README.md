...
6. **📊 統計分析、雙軸即時篩選與五大 Sheet 結構化報表匯出 (Analytics & Reporting)**
   - **題目指標分析**：有效作答數、未作答數、作答率、選項分佈、數值描述統計（Mean, Median, Range, Sample SD）。
   - **雙軸多維篩選**：支援狀態（`全部/已完成/填寫中`）與時間（`全時段/近30日/近7日/今日`）即時連動。
   - **2-Way 交叉分析**：支援任意兩題交叉運算，內建 `MIN_CELL_SIZE = 5` 與互補抑制（Complementary Suppression）隱私遮蔽防護。
   - **多工作表 Excel 匯出**：匯出檔案包含 `匯出資訊 (Meta)`、`填答總覽 (Responses)`、`作答明細 (Answers)`、`題目統計摘要 (Summary)` 與 `題目與選項設定` 5 大 Sheet，自動套用當前篩選條件。
   - **列印友善報表**：原生 `@media print` 樣式，一鍵產出具備問卷標頭、篩選說明與 KPI 彙整之專業報表。
7. **🧪 420+ 項自動化測試保證極致穩定**
   - 涵蓋 42 個測試套件（Excel 雙向 Round-trip、確定性統計、多租戶隔離、RBAC 角色邊界、安全迴歸、純函數統計引擎與匯出報表測試，427/427 全數通過）。
